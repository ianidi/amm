...
library Fixed192x64Math {
    ...
    function exp(int256 x) public pure returns (uint256) {
        ...
        (uint256 lower, uint256 upper) =
            pow2Bounds((x * int256(ONE)) / int256(LN2));
    ...
    function pow2(int256 x, EstimationMode estimationMode)
        public
        pure
        returns (uint256)
    {
        ...
        returns (uint256 lower, uint256 upper)
    {
        ...
        if (shift >= 0) {
            if (result >> (256 - shift) == 0) {
                lower = uint256(result) << shift;
                zpow <<= shift; // todo: is this safe?
                if (lower + uint256(zpow) >= lower)
                    upper = lower + uint256(zpow);
                else upper = 2**256 - 1;
                return (lower, upper);
            } else return (2**256 - 1, 2**256 - 1);
        }
        zpow = (zpow >> (-shift)) + 1;
        lower = uint256(result) >> (-shift);
        upper = lower + uint256(zpow);
    ...
    function binaryLog(uint256 x, EstimationMode estimationMode)
        public
        pure
        returns (int256)
    {
    ...
    /// }
    function log2Bounds(uint256 x)
        public
        pure
        returns (int256 lower, int256 upper)
    {
        require(x > 0);
        // compute ⌊log₂x⌋
        lower = floorLog2(x);
