...
contract FixedProductMarketMaker is ERC20, ERC1155TokenReceiver {
    ...
    function feesWithdrawableBy(address account) public view returns (uint256) {
        uint256 rawAmount =
            feePoolWeight.mul(balanceOf(account)) / totalSupply();
    ...
    function _beforeTokenTransfer(
        ...
        uint256 withdrawnFeesTransfer =
            totalSupply == 0 ? amount : feePoolWeight.mul(amount) / totalSupply;
        ...
        external
    {
        ...
        if (poolShareSupply > 0) {
            ...
            for (uint256 i = 0; i < poolBalances.length; i++) {
                uint256 remaining =
                    addedFunds.mul(poolBalances[i]) / poolWeight;
            ...
            if (distributionHint.length > 0) {
                ...
                for (uint256 i = 0; i < distributionHint.length; i++) {
                    uint256 remaining =
                        addedFunds.mul(distributionHint[i]) / maxHint;
        ...
        returns (uint256)
    {
        ...
        uint256[] memory poolBalances = getPoolBalances();
        uint256 investmentAmountMinusFees =
            investmentAmount.sub(investmentAmount.mul(fee) / ONE);
